...
export default function Login() {
  const { login, user, setShowWelcome } = useAuth();
  ...
  // Quando o user estiver disponível no contexto, vai pro dashboard
  // Funciona tanto no login normal quanto no refresh (já logado)
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Ativa o overlay de boas-vindas no AppLayout (não depende de navigação)
      setShowWelcome(true);
      // A navegação para "/" será disparada automaticamente pelo useEffect([user])
      // quando o AuthContext detectar o usuário via onAuthStateChanged
    } catch {
